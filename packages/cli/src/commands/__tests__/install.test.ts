...
describe("install command", () => {
  ...
  beforeEach(async () => {
    ...
    skillInstallerGenerateLockFileSpy = vi.spyOn(
      SkillInstaller.prototype,
      "generateLockFile"
    );
  });
  ...
  describe("Basic Installation", () => {
    it("should install skills from package.json", async () => {
      ...
      skillInstallerInstallSpy.mockImplementation(
        async (name: string, spec: string) => {
      ...
      expect(skillInstallerInstallSpy).toHaveBeenCalledWith(
        "skill-one",
        "github:user/skill-one"
      );
      expect(skillInstallerInstallSpy).toHaveBeenCalledWith(
        "skill-two",
        "github:user/skill-two"
      );
  ...
  describe("Lock File Generation", () => {
    it("should generate lock file only with successful installs", async () => {
      ...
      const lockFileArg = skillInstallerGenerateLockFileSpy.mock
        .calls[0][0] as Record<string, InstallResult>;
  ...
  describe("Edge Cases", () => {
    it("should handle skills with special characters in names", async () => {
      ...
      expect(skillInstallerInstallSpy).toHaveBeenCalledWith(
        "skill-with-dashes",
        "github:user/skill-with-dashes"
      );
      expect(skillInstallerInstallSpy).toHaveBeenCalledWith(
        "@scoped/skill",
        "github:org/scoped-skill"
      );
      expect(processExitSpy).toHaveBeenCalledWith(0);
    });
